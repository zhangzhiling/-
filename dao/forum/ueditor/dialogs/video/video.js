...
(function(){
    ...
    UM.registerWidget( widgetName,{
        ...
        createPreviewVideo: function(url){
            ...
            var me = this;
            var lang = me.lang;
            var conUrl = me.convert_url(url);
            var arrUrl = conUrl.split("?");  //判断视频id参数在末尾跟着的情况

            if(!me.endWith(arrUrl[0],[".swf",".flv",".wmv",".f4v"])){
                $("#eduiVideoPreview", me.$widget).css("color","red").html( lang.urlError );
                return;
            }
        ...
        insertSingle: function(){
            ...
            if ($("#eduiVideoPreview", me.$widget).html()==lang.urlError) return false;
         ...
         * URL转换
         */
        convert_url: function(url){  //无法支持爱奇艺，因其地址不暴露视频ID
            if ( !url ) return '';
            var matches = url.match(/youtu.be\/(\w+)$/) ||
                    url.match(/youtube\.com\/watch\?v=(\w+)/) ||
                    url.match(/youtube.com\/v\/(\w+)/);
            var youku = url.match(/youku\.com\/v_show\/id_(\w+)/); 
            var youkuPlay = /player\.youku\.com/ig.test(url);
            var bilibili = url.match(/www.bilibili.com\/video\/av([\d]+)\/?/i);  //哔哩哔哩
            var leshi = url.match(/le\.com\/ptv\/vplay\/([\d]+)\.html/i);   //乐视
            ...
            if(youkuPlay){
                ...
                url = "http://static.hdslb.com/miniloader.swf?aid="+bilibili[1];
            }else if(leshi){
            	url = "http://i7.imgs.letv.com/player/swfPlayer.swf?id="+leshi[1]+"&autoplay=0";
            }
            return url;
        },
        /**

...
var getTencentSignUrl = "/forum/ueditor/sign"; //获取腾讯云签名
var routePath = "/forum";  //路由名称
var logOutUrl="/user/logout";
$(function(){
    $(document).popErrorF({type:"init"});//初始化提示层
    $(document).popDeleteF({type:"init"});//初始化删除提示层
})
$.fn.extend({
	popF:function(option){
		var _this = $(this);
	...
	//发帖， 回帖 弹层
	popPostF:function(str){
		var $this = $(this);
		if(typeof str == "string"){
			switch (str){
			case "open":
				$this.parent(".pop_f_wrap").show();
				break;
			case "close":
				$this.parent(".pop_f_wrap").hide();
				break;
			default:
				break;
	...
	//删除确认层
	popDeleteF:function(option){
		var defaults = {
				str:''+   //字符串可自定义
						'<div class="popErrorTip js_popDeleteTip" style="display:none;">'+
							'<div class="popErrorCon">'+
								'<div class="img"></div>'+
								'<p class="js_pop_delete_tip">确定要删除吗？</p>'+
								'<div class="btn_wrap">'+
									'<a class="btn js_confirmDeleBtn">确定</a>'+
									'<a class="btn giveUp js_closeDeletePop">取消</a>'+
								'</div>'+

							'</div>'+
							'<div class="maskErrorD"></div>'+
						'</div>',
				type:"init",  //inint:初始化 如type为init不需要再传其他参数。  open:打开弹层  close:关闭弹层 外部也可调用关闭功能
				tip:"确定要删除吗？",  //警告层提示语
				confirmFn:function(){ 
					//确认按钮执行函数
			...
			switch(defaults.type){
				case "init":
					...
					break;
				case "open":
					$(".js_pop_delete_tip").html(defaults.tip);
					$(".js_popDeleteTip").show();
					//确认按钮
					$(document).off("click",".js_confirmDeleBtn").on("click",".js_confirmDeleBtn",function(){
						if(defaults.confirmFn){
							defaults.confirmFn();
						}
					});
					//取消按钮
					$(document).off("click",".js_closeDeletePop").on("click",".js_closeDeletePop",function(){
						$(document).popDeleteF({type:"close"});
						if(defaults.closeFn){
							defaults.closeFn();
						}
					});
  ...
  logOutFn:function(){
    ...
    //获取腾讯云签名
    getTencentSignFn:function(uploadFn){
		//腾讯云域名
		var tencentUrl = "http://tj.file.myqcloud.com";
		
    	$.ajax({
    		url:getTencentSignUrl,
    		type:"get",
    		dataType:"json",
    		data:{
    			r:Math.random()

/*-------------------------------------------------------------------------
* add by tht 2017-6-20
* for forum info ajax page
-------------------------------------------------------------------------*/
$.fn.extend({
    //ajax分页初始化方法
    ajaxPage: function (options) {
        ...
        var
            setting = {
                ...
                //ajax成功返回，处理数据之前操作：如，需要跳转错误页
                beforeDoDataFn:function(data){
                	
                },
                editorObj:null,
                //当数据遍历好并分页完成后执行函数
                successFn: function (allCount, pageSize, pageIndex, editorObj) {
                },
                //点击分页按钮时处理函数
        ...
        var ajaxSuc = function (data) {
            if (data) {
            	if(setting.beforeDoDataFn){
            		setting.beforeDoDataFn(data,$this);
            	}
                if (setting.dataFn) {
                ...
                //当前页码
                var callBackPageIndex = setting.callBackPageIndex ? eval(setting.callBackPageIndex) : null;
                ...
                setting.pIndex = callBackPageIndex ? callBackPageIndex : pageIndex;  //callBackPageIndex:是服务端返回pageIndex
                //分页
                ...
                if (setting.successFn) {
                    setting.successFn(allCount, pageSize, pageIndex,setting.editorObj);  //editorObj:贴吧编辑器实例
                }
            }
        };
    ...
    pagesFn: function (setting) {
        var
            ...
            //分页按钮html容器
            pagehtml = '<a ' + (isfirstpg ? 'class="prev disabled"' : 'class="prev"') + ' href="javascript:;"  pgid="-4">' + btnTxt.prev + '</a>'+
        	'<a ' + (isfirstpg ? 'class="curpage"' : '') + ' href="javascript:;" pgid="-3">' + btnTxt.first + '</a>';
        ...
        if (allPage < 6) {//如果页面小于6页时，显示全部页码
            ...
            var isLast = (pageIndex + 2) < allPage,
                aDot = '<a class="dot" href="javascript:;">...</a>';
        ...
        pagehtml += '<a ' + (lastpg ? 'class="curpage"' : '') + ' href="javascript:;" pgid="-2">' + btnTxt.last + '</a>' +
                                 '<a ' + (lastpg ? 'class="next disabled"' : 'class="next"') + ' href="javascript:;" pgid="-1">' + btnTxt.next + '</a>';

        //先清空分页容器
        pageObj.empty().hide(); 
        if(setting.allPage>1){ //只有一页，不显示分页
            //装载分页按钮
        	pageObj.show().html(pagehtml);
          //为分页按钮绑定事件

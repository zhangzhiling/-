/*
 * add by tht 2017-10-30
 * for other user Dynamic function
 */
/*=======================update log start=====================*/
...
(function(){
	...
	var getOtherDynamicAllUrl = "/user/feedList";  //获取他人动态接口    参数type； all：全部类型动态
	var getThreadInfoUrl = "/forum/thread/info/ofThread";  //获取主贴内容接口
	var getPostInfoUrl = "/forum/post/info/ofPost"; //获取回帖内容接口
	var getCommentInfoUrl = "/forum/comment/info/ofComment";//获取评论内容接口
	...
	function OtherUserDynamic(){
		//Constructor
	}
	OtherUserDynamic.prototype = {
		init:function(type){
			var _this = this;
			//获取全部动态
			_this.getDynamicListFn(type);

		},
		//获取他人动态列表
		getDynamicListFn:function(type){
			var _this = this;
			
			$(".js_allMs").ajaxPage({
                url:getOtherDynamicAllUrl,
                type: "GET",
                ...
                paramObj: {//传参
                    type:type,   //他人动态类型  all:全部   replyThread:回帖
                     ...
                     if(d.status=="success"){
                    	 ...
                    	 if(datas&&datas.length>0){
                    		 ...
                    		 for(var i=0,l =datas.length ;i<l;i++){
                    			 ...
                    			 var dynamicType=datas[i].type;//帖子类型
                    			 
                    			 //获取信息内容
                    			 switch (dynamicType){
                    			    //主贴
                    			 	case "addThread":
                    			 		var forumHtml = _this.getThreadInfoFn(datas[i]).forumStr;
                    			 		htmlLi += forumHtml;
                    			 		continue;
                    			 	//回帖
                    			 	case "replyThread":
                    			 		var postHtml = _this.getPostInfoFn(datas[i],1,null).postStr;//1无点击状态，0点击状态
                    			 		htmlLi += postHtml;
                    			 		continue;
                    			 	//评论
                    			 	case "replyComment":
                    			 	case "replyPost":
                    			 		var commentHtml = _this.getCommentInfoFn(datas[i],null,1,null);//1无点击状态，0点击状态
                    			 		htmlLi += commentHtml;
                    			 		continue;
                    			 	//送礼
                    			 	case "gift":
                    			 		var giftHtml = _this.getGiftInfoFn(datas[i]);
                    			 		htmlLi += giftHtml;
                    			 		continue;
                    			 	default:
                    			 		continue;
                    		 ...
                    		 $(".comingSoon").html("Ta暂无任何动态").css("display","block");
                    	 }
			 ...
			 $.ajax({
				 ...
				 success:function(d){
					 ...
					 if(d.status=="success"){
						 ...
						 var expVideoForum = new RegExp('(<EMBED|<embed)[^>]+(>|\/>)','g');  //视频个数正则表达式
						 var expImgForum = new RegExp('(<IMG|<img)(?![^>]+class="emotion")(?![^>]+src="http://wanwd.gyyx.cn/Content/js/editor/dialogs/emotion)[^>]+(>|\/>)','g'); //图片个数正则表达式
						
					 
						 var time = listData.createTime?listData.createTime:"";  //时间
		                ...
		                //视频区,图片展示
						var videoImgArr=[];
						var videos=(datas.content).match(expVideoForum); 
						var Imgs=(datas.content).match(expImgForum);
		                 ...
		                 forumStr+='<li class="dynamicAll dynamic_li">'+
									...
									'<div class="third_line">'+
										...
										'<div class="about_num">'+
											...
											'<span class="line">|</span>'+
											'<span class="zan js_zan" data_threadid="'+datas.id+'" data_contentyype="post" data_contentid="'+datas.postId+'" >'+zanNum+'</span>'+
										'</div>'+
									'</div>'+
								  '</li>';
					 }
				 ...
				 success:function(d){
					 ...
					 postStr+='<li class="dynamicAll dynamic_li">'+
								'<div class="first_line">'+
									...
									'<div class="fun_btn_wrap">'+
										'<span class="forThisReply"><a target="_blank" class="js_postLink" content="' + postId + '" _href="/forum/thread?id='+datas.threadId+'#'+postId+'">回复</a></span>'+
										'<span class="forThisShare" style="display:none"></span>'+
									'</div>'+
								'</div>'+
								'<div class="second_line">'+
							...
							'</div>'+
								'<div class="third_line">'+
									...
									'<div class="about_num">'+
										...
										'<span class="line">|</span>'+
										'<span class="zan js_zan" data_threadid="'+datas.threadId+'" data_contentyype="post" data_contentid="'+postId+'" >'+likeNums+'</span>'+
									'</div>'+
								'</div>'+
							  '</li>';
						 }
						 ...
						 if(d.status!="hiddenComment" && d.status!="deleteComment" && d.data){
								...
								commentStr+='<li class="dynamicAll dynamic_li" dao='+id+'>'+
												'<div class="first_line">'+
													...
													'<div class="fun_btn_wrap">'+
														'<span class="forThisReply"><a target="_blank" class="js_commentLink" comment="'+commentId+'" _href="/forum/thread?id='+threadId+'#'+postId+'">回复</a></span>'+
														'<span class="forThisShare" style="display:none"></span>'+
													'</div>'+
												'</div>'+
												'<div class="second_line">'+
												...
												'<div class="third_line">'+
													'<div class="time">'+createTime+'</div>'+
													'<div class="about_num">'+
														'<span class="zan js_zan" data_threadid="'+threadId+'" data_contentyype="comment" data_contentid="'+commentId+'" data_postid="'+postId+'"  >'+likeNums+'</span>'+
													'</div>'+
												'</div>'+
											  '</li>';
        ...
        caiAndZanAjaxFn:function(obj,type){
        	var dataJson={
    			r:Math.random(),
				threadId:obj.attr("data_threadid"),
				contentType:obj.attr("data_contentyype"),
				contentId:obj.attr("data_contentid"),
				poke:type
        	};
        	if(obj.attr("data_contentyype")=="comment"){
        		dataJson.postId=obj.attr("data_postid")
        	}
        	$.ajax({
	...
	window.otherUserDynamic = function(){
		return new OtherUserDynamic();
	};
})();
$(function(){
	//动态二级分类展示
	...
	$(".js_dynamicTypes li").click(function(){
		if($(".typeMsg").attr("repeat")=="no"){
		 ...
		 if(_thisType==""){
     		...
     		otherUserDynamic().init(_thisType);
     	}
    ...
    $("body").on("click",".dynamic_li .js_postLink",function(){
    	...
    	otherUserDynamic().getPostInfoFn(contentId,0,_href,$(this));
    })
	 //他人动态列表评论信息点击事件
    $("body").on("click",".dynamic_li .js_commentLink",function(){
    	...
    	otherUserDynamic().getCommentInfoFn(contentId,daoId,0,_href,$(this));
    })
	otherUserDynamic().init("all");	
});